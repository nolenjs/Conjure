...
class ConjureProgram
{
    ...
    static void OpenDownload(string dest)
    {
        //tempIP, tempSuccess, tempURL, tempUser, tempTime, tempMsg, tempType, tempDest
        Console.WriteLine("entering in open download");
        // Veronica's Test: /home/future-leader/Documents/RnD/Websites/gnome-sudoku
        ConjureLog log1 = new ConjureLog(); // One log per try/catch statement
        ...
        try{
            ...
            FileSystemRights fRights = FileSystemRights.ReadAndExecute;
            AccessControlType type = AccessControlType.Allow;
            string access = "Everyone";
            FileSystemAccessRule rule = new FileSystemAccessRule(access, fRights, type);
            ...
            log1.success = "failure";
            Console.WriteLine(e.Message);
            log1.message = "Set Permissions: " + e.GetType().ToString().Split('.')[1];
            Logs.Add(log1);
        }

        try
        {
            System.Diagnostics.Process.Start("MONSIEUR_LINUX");
            log2.success = "success";
            log2.message = "none";
            Logs.Add(log2);
            return;
    ...
    static int GetRandScrollSpeed()
        ...
        try {
            ...
            if (message == "pause"){
                ...
                DownloadFromWeb("/home/future-leader/ACE/conjure/conjure-Quillen/ConjureSol", urlStr);
            }
        ...
        static void sendLogs(Socket sender){
            ...
            // for(int p=0; p<logList.Count; p++){
            //     Console.WriteLine("entered for loop");
            //     for (int q=0; q<logList[p].Count; q++){
            //         Console.WriteLine(logList[p][q]);
            //     }
            // }
